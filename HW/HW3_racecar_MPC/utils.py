...
def apply_action(pb_client, body_id, linear_velocity, angular_velocity):
    ...
    if (np.abs(linear_velocity) > .0001):
        steering_angle = np.arctan(car_length*angular_velocity/linear_velocity)
    
        for joint_index in joint_indices['steering_hinge']:
            joint_position_control(pb_client, body_id, joint_index, steering_angle)

    wheel_velocity_rear_left = (linear_velocity/wheel_radius) * (1 - (car_width/(2*car_length))*np.tan(steering_angle))
    wheel_velocity_rear_right = (linear_velocity/wheel_radius) * (1 + (car_width/(2*car_length))*np.tan(steering_angle))
